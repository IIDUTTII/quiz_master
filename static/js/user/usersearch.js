const UserSearchPage = {
    ...
    data() {
        return {
            ...
            searchQuery: '',
            results: { subjects: [], quizzes: [], chapters: [] },
            totalResults: 0,
            isLoading: false,
            err: null
        };
    ...
    methods: {
        async performSearch(query = null) {
            ...
            if (!searchTerm.trim()) {
                this.results = { subjects: [], quizzes: [], chapters: [] };
                this.totalResults = 0;
                return;
            }
            this.isLoading = true;
            this.err = null;
            try {
                const resp = await fetch(`/user/page/search?q=${encodeURIComponent(searchTerm)}`);
                const data = await resp.json();
                if (data.success) {
                    this.results = data.results || {};
                    this.totalResults = data.total_results || 0;
                } else {
                    this.err = data.message || 'Search failed';
                }
            } catch (error) {
        ...
        <div class="row">
            <div class="col-12">
                ...
                <div v-if="!searchQuery" class="alert alert-info mt-3">
                    ...
                    Enter a search term in the navbar to find subjects, quizzes, or chapters
                </div>
